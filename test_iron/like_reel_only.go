...
var reactionIDslike_reel_only = map[string]string{
...
func generateLikeMetalike_reel_only(postID string) (string, string, string, string) {
...
func randomExcellentBandwidthlike_reel_only() string {
...
func fetchOwnerIDlike_reel_only(objectID, token string) (string, error) {
...
func extractIDFromLinklike_reel_only(link string) string {
	...
	for _, p := range parts {
		if len(p) > 10 && isNumericlike_reel_only(p) {
			return p
		}
	}
	return ""
...
func isNumericlike_reel_only(s string) bool {
	for _, r := range s {
		if r < '0' || r > '9' {
			return false
		}
...
func Runlike_reel_only(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	err = db.QueryRow("SELECT reaction_type, link FROM like_reel_only_table LIMIT 1").Scan(
		&reactionType, &link)
	if err != nil {
		fmt.Println("❌ ดึงข้อมูล like_reel_only_table ไม่สำเร็จ: " + err.Error())
		return
	}

	postID := extractIDFromLinklike_reel_only(link)
	if postID == "" {
		fmt.Println("❌ ดึง postID จากลิงก์ไม่สำเร็จ: " + link)
		return
	}

	ownerID, err := fetchOwnerIDlike_reel_only(postID, accessToken)
	if err != nil {
		fmt.Println("❌ ดึง owner_id ไม่สำเร็จ: " + err.Error())
		return
	}

	reactionID := reactionIDslike_reel_only[reactionType]
	feedbackIDB64, clientMutationID, timestamp, qid := generateLikeMetalike_reel_only(postID)
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthlike_reel_only())
	...
	//	_, err = db.Exec("DELETE FROM like_reel_only_table WHERE reaction_type = ?", reactionType) //db.Exec("DELETE FROM uid_table WHERE user_id = ?", uid) reactionType, link
	//	if err != nil {
	...
	_, err = db.Exec("DELETE FROM like_reel_only_table WHERE link = ?", link) //db.Exec("DELETE FROM uid_table WHERE user_id = ?", uid)
	if err != nil {
		fmt.Println("❌ ลบไม่สำเร็จ:", err)
	} else {
	...
	_, err = db.Exec("INSERT INTO respond_for_like_reel_only_table (respond_txt) VALUES (?)", string(bodyResp))
	if err != nil {
		fmt.Println("❌ บันทึก response ลงตาราง respond_for_like_reel_only_table ไม่สำเร็จ:", err)
	} else {
		fmt.Println("💾 บันทึก response แล้วลงตาราง respond_for_like_reel_only_table")
	}
