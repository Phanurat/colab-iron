...
func extractPostIDpreC0mment5_FetchPredictiveTextSuggestions(rawurl string) (string, error) {
	re := []*regexp.Regexp{
...
func generateFeedbackIDpreC0mment5_FetchPredictiveTextSuggestions(postID string) string {
	return base64.StdEncoding.EncodeToString([]byte("feedback:" + postID))
}

func generateUUIDpreC0mment5_FetchPredictiveTextSuggestions() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment5_FetchPredictiveTextSuggestions() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}

func generateHex32preC0mment5_FetchPredictiveTextSuggestions() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment5_FetchPredictiveTextSuggestions(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	//	textValue := "hkh"
	connToken := generateHex32preC0mment5_FetchPredictiveTextSuggestions()
	...
	if err != nil {
		panic("❌ ดึงข้อมูล like_comment_and_reply_comment_table ไม่สำเร็จ: " + err.Error())
	}

	postID, err := extractPostIDpreC0mment5_FetchPredictiveTextSuggestions(link)
	if err != nil {
		panic("❌ ขุด postID ล้มเหลว: " + err.Error())
	}
	feedbackID := generateFeedbackIDpreC0mment5_FetchPredictiveTextSuggestions(postID)
	traceID := generateUUIDpreC0mment5_FetchPredictiveTextSuggestions()

	variables := fmt.Sprintf(`{"queryParams":{"text":{"sensitive_string_value":"%s"},"surface":"comment","is_mobile":true,"feedback_id":"%s"}}`, link, feedbackID)
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment5_FetchPredictiveTextSuggestions())
