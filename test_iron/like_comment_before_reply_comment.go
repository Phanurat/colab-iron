...
func initlike_comment_before_reply_comment() {
...
func randomExcellentBandwidthlike_comment_before_reply_comment() string {
	min, max := 20_000_000, 35_000_000
	return strconv.Itoa(rand.Intn(max-min+1) + min)
}

func isNumericlike_comment_before_reply_comment(s string) bool {
	for _, r := range s {
		if r < '0' || r > '9' {
			return false
		}
...
func randIntlike_comment_before_reply_comment() int {
	return int(time.Now().UnixNano() % 100_000_000)
}

// ---------- FACEBOOK ID EXTRACTION ----------

func extractCommentIDlike_comment_before_reply_comment(rawURL string) (string, error) {
...
func extractFacebookIDslike_comment_before_reply_comment(rawURL string) (ownerID, postID string, err error) {
	u, err := url.Parse(rawURL)
	if err != nil {
		return "", "", err
	}
	...
	if ownerID == "" {
		if m := reUsername.FindStringSubmatch(rawURL); len(m) > 1 {
			username := m[1]
			if isNumericlike_comment_before_reply_comment(username) {
				ownerID = username
			} else {
				ownerID, err = getFBIDFromUsernamelike_comment_before_reply_comment(username)
				if err != nil {
					return "", "", err
				}
			}
...
func getFBIDFromUsernamelike_comment_before_reply_comment(username string) (string, error) {
...
type MetaDatalike_comment_before_reply_comment struct {
	PageIDlike_comment_before_reply_comment   string
	ownerIDlike_comment_before_reply_comment  string
	Trackinglike_comment_before_reply_comment []interface{}
}

func fetchMetalike_comment_before_reply_comment(postID, commentID, token string) *MetaDatalike_comment_before_reply_comment {
	var endpoint string
	if commentID != "" {
	...
	if err != nil {
		...
		return &MetaDatalike_comment_before_reply_comment{Trackinglike_comment_before_reply_comment: []interface{}{}}
	}
	defer resp.Body.Close()
	...
	return &MetaDatalike_comment_before_reply_comment{
		PageIDlike_comment_before_reply_comment:   data.From.ID,
		ownerIDlike_comment_before_reply_comment:  data.From.ID,
		Trackinglike_comment_before_reply_comment: []interface{}{},
	}
}
...
func generateFeedbackIDlike_comment_before_reply_comment(id string) string {
	return base64.StdEncoding.EncodeToString([]byte("feedback:" + id))
}

// generateAttributionID สร้างค่า attribution_id_v2 แบบไม่ซ้ำ
func generateAttributionIDlike_comment_before_reply_comment() string {
...
var reactionIDslike_comment_before_reply_comment = map[string]string{
...
func Runlike_comment_before_reply_comment(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	err = db.QueryRow("SELECT reaction_type, link FROM like_comment_and_reply_comment_table LIMIT 1").Scan(&reactionType, &link)
	if err != nil {
		fmt.Println("❌ ดึงลิงก์จาก like_comment_and_reply_comment_table ไม่สำเร็จ: " + err.Error())
		return
	}
	...
	_, postID, err := extractFacebookIDslike_comment_before_reply_comment(link)
	if err != nil {
		fmt.Println("❌ ขุด owner_id/post_id ไม่สำเร็จ: " + err.Error())
		return
	}
	commentID, _ := extractCommentIDlike_comment_before_reply_comment(link)
	...
	feedbackID := generateFeedbackIDlike_comment_before_reply_comment(targetID)

	// 4. ดึง meta
	meta := fetchMetalike_comment_before_reply_comment(postID, commentID, accessToken)

	// 5. เตรียม variables
	mutationID := uuid.New().String()
	traceID := uuid.New().String()
	attribution := generateAttributionIDlike_comment_before_reply_comment() ////////////////////////////////////////
	actionTimestamp := time.Now().Unix()
	reactionID := reactionIDslike_comment_before_reply_comment[reactionType]

	variables := map[string]interface{}{
		"input": map[string]interface{}{
			"tracking":             meta.Trackinglike_comment_before_reply_comment,
			...
			"page_id":              meta.PageIDlike_comment_before_reply_comment,
			"content_owner_id_new": meta.ownerIDlike_comment_before_reply_comment,
		},
	}
	...
	req.Header.Set("x-fb-privacy-context", meta.PageIDlike_comment_before_reply_comment)
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthlike_comment_before_reply_comment())
	...
	_, err = db.Exec("INSERT INTO respond_for_like_comment_before_reply_comment_table (respond_txt) VALUES (?)", string(bodyResp))
	if err != nil {
		fmt.Println("❌ บันทึก response ลงตาราง respond_for_like_comment_before_reply_comment_table ไม่สำเร็จ:", err)
	} else {
		fmt.Println("💾 บันทึก response แล้วลงตาราง respond_for_like_comment_before_reply_comment_table")
	}

	//	_, err = db.Exec("DELETE FROM like_comment_before_reply_comment_table WHERE reaction_type = ?", reactionType) //db.Exec("DELETE FROM uid_table WHERE user_id = ?", uid) reactionType, link
	//	if err != nil {
	//		fmt.Println("❌ ลบไม่สำเร็จ:", err)
	//	} else {
	//		fmt.Println("🧹 ลบ reaction_type ออกจากฐานข้อมูลแล้ว:", reactionType)
	//	} respond_for_like_comment_before_reply_comment_table

	//	_, err = db.Exec("DELETE FROM like_comment_before_reply_comment_table WHERE link = ?", link) //db.Exec("DELETE FROM uid_table WHERE user_id = ?", uid)
	//	if err != nil {
	//		fmt.Println("❌ ลบไม่สำเร็จ:", err)
	//	} else {
