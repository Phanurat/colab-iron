...
func extractPostIDpreC0mment3_for_comment_comment(rawurl string) (string, error) {
	re := []*regexp.Regexp{
...
func generateFeedbackIDpreC0mment3_for_comment_comment(postID string) string {
	return base64.StdEncoding.EncodeToString([]byte("feedback:" + postID))
}

func generateUUIDpreC0mment3_for_comment_comment() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment3_for_comment_comment() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}
func generateHex32preC0mment3_for_comment_comment() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment3_for_comment_comment(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	connToken := generateHex32preC0mment3_for_comment_comment()
	...
	if err != nil {
		panic("❌ ดึง app_profiles ไม่ได้: " + err.Error())
	}

	var link string
	err = db.QueryRow(`SELECT link FROM like_comment_and_reply_comment_table LIMIT 1`).Scan(&link)
	if err != nil {
		panic("❌ ดึงลิงก์ไม่สำเร็จ: " + err.Error())
	}

	postID, err := extractPostIDpreC0mment3_for_comment_comment(link)
	if err != nil {
		panic("❌ ขุด postID ล้มเหลว: " + err.Error())
	}
	feedbackID := generateFeedbackIDpreC0mment3_for_comment_comment(postID)
	traceID := generateUUIDpreC0mment3_for_comment_comment()
	clientMutationID := generateUUIDpreC0mment3_for_comment_comment()
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment3_for_comment_comment())
