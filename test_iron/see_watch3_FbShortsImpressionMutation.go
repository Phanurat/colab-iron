...
func randomExcellentBandwidthsee_watch3_FbShortsImpressionMutation() string {
...
func Runsee_watch3_FbShortsImpressionMutation(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	fmt.Println("🌐 ใช้ Proxy:", proxyAddr) // ✅ เพิ่ม debug แสดง proxy ที่ใช้อยู่
	// ----- GENERATE ค่า Dynamic -----
	clientMutationID := uuid.New().String()

	host := "graph.facebook.com"
	//	address := host + ":443"

	baseID := int64(24740742578859242)
	offset := rand.Int63n(10000000) // ไม่ควรเกินเยอะ เดี๋ยวเลยช่วงเวลา
	fakeID := baseID + offset
	fmt.Println(fakeID)
	videoID := fakeID
	...
	variables := map[string]interface{}{
		"input": map[string]interface{}{
			...
			"video_ids":          []string{strconv.FormatInt(videoID, 10)},
			"actor_id":           userID,
		},
	}
	...
	data.Set("fb_api_req_friendly_name", "FbShortsImpressionMutation")
	data.Set("fb_api_caller_class", "graphservice")
	data.Set("client_doc_id", "268378482815649707521961144754")
	data.Set("variables", string(variablesJSON))
	data.Set("fb_api_analytics_tags", `["GraphServices"]`)
	...
	req.Header.Set("X-FB-Friendly-Name", "FbShortsImpressionMutation")
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthsee_watch3_FbShortsImpressionMutation())
