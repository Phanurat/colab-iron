...
func extractPostIDpreC0mment4_for_comment_comment(rawurl string) (string, error) {
	re := []*regexp.Regexp{
...
func generateFeedbackTargetIDpreC0mment4_for_comment_comment(postID string) string {
	id := "feedback:" + postID
	return base64.StdEncoding.EncodeToString([]byte(id))
}

func generateUUIDpreC0mment4_for_comment_comment() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment4_for_comment_comment() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}

func generateHex32preC0mment4_for_comment_comment() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment4_for_comment_comment(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	host := "graph.facebook.com"
	//	address := host + ":443"
	clientDocID := "40414289251135926563767751059"
	friendlyName := "MentionsSuggestionQuery"
	connToken := generateHex32preC0mment4_for_comment_comment()
	...
	postID, err := extractPostIDpreC0mment4_for_comment_comment(link)
	if err != nil {
		fmt.Println("❌ ขุด postID ไม่สำเร็จ: " + err.Error())
		return

	}
	feedbackTargetID := generateFeedbackTargetIDpreC0mment4_for_comment_comment(postID)
	traceID := generateUUIDpreC0mment4_for_comment_comment()
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment4_for_comment_comment())
