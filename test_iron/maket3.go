...
func randomExcellentBandwidthmaket3() string {
...
func Runmaket3(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	variables := genVariablesmaket3()
	fmt.Println("🧾 Variables:")
	fmt.Println(prettyPrintJSONmaket3([]byte(variables)))

	form := url.Values{}
	form.Set("access_token", accessToken)
	form.Set("fb_api_caller_class", "RelayModern")
	form.Set("fb_api_req_friendly_name", "MarketplaceHomeFeedQueryRendererQuery")
	form.Set("variables", variables)
	form.Set("server_timestamps", "true")
	form.Set("doc_id", "6816767618392859")
	...
	req.Header.Set("privacy_context", "MarketplaceHomeRoute")
	req.Header.Set("x-fb-client-ip", "True")
	req.Header.Set("X-FB-Connection-Type", "MOBILE.HSDPA")
	req.Header.Set("x-fb-device-group", devicegroup)
	req.Header.Set("X-FB-Friendly-Name", "RelayFBNetwork_MarketplaceHomeFeedQueryRendererQuery")
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthmaket3())
...
func genVariablesmaket3() string {
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	postID := fmt.Sprintf("7%d", r.Int63n(1e16))
	qid := fmt.Sprintf("-%d", r.Int63n(1e17)+1e16)
	rankSig := fmt.Sprintf("%d", r.Int63n(1e17)+1e16)
	...
	rankObj := map[string]interface{}{
		...
		"candidate_retrieval_source_map": map[string]int{
			...
			"8919379358120173":  3001,
			"9592389307550917":  805,
		},
		"grouping_info": nil,
	}

	seenTracking := map[string]interface{}{
		"qid":               qid,
		"mf_story_key":      postID,
	...
	vars := map[string]interface{}{
		...
		"realtimeSignalStore": map[string]interface{}{
			"clickData":          []interface{}{},
			"dwellData":          []interface{}{},
			"homeFeedFcrt":       false,
		...
		"seenItemTracking":                     seenTracking,
		"shouldSkipAdRequest":                  false,
	...
	buf, _ := json.Marshal(vars)
	return string(buf)
}

func prettyPrintJSONmaket3(input []byte) string {
