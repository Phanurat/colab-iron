...
func generateNavChainRunset_status(actorID string) string {
...
func randomExcellentBandwidthRunset_status() string {
...
func Runset_status(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	var status string                                                                        //statusLink
	err = db.QueryRow("SELECT status_text FROM set_status_text_table LIMIT 1").Scan(&status) //, &statusLink) //, status_link
	if err != nil {
		fmt.Println("❌ ดึงข้อมูล set_status_text_table ไม่สำเร็จ: " + err.Error())
		return
	}
	...
	variablesObj := map[string]interface{}{
		"input": map[string]interface{}{
			...
			"composer_session_id": uuidRunset_status(),
			"idempotence_token":   "FEED_" + uuidRunset_status(),
			"client_mutation_id":  uuidRunset_status(),
			"actor_id":            userID,
			"audiences": []interface{}{
	...
	req.Header.Set("x-fb-ta-logging-ids", fmt.Sprintf("graphql:%s", uuidRunset_status()))
	...
	req.Header.Set("X-FB-Navigation-Chain", generateNavChainRunset_status(userID))
	req.Header.Set("x-fb-net-hni", netHni)                                               // เพิ่มเข้าไป
	req.Header.Set("x-fb-sim-hni", simHni)                                               // เพิ่มเข้าไป
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthRunset_status()) //เพิ่มเข้าไป
	...
	_, err = db.Exec("DELETE FROM set_status_text_table WHERE status_text = ?", status) //db.Exec("DELETE FROM uid_table WHERE user_id = ?", uid)
	if err != nil {
		fmt.Println("❌ ลบไม่สำเร็จ:", err)
	} else {
		fmt.Println("🧹 ลบ set_status_text_table ออกจากฐานข้อมูลแล้ว:", status)
	}

}

func uuidRunset_status() string {
	return fmt.Sprintf("%08x-%04x-%04x-%04x-%012x",
		rand32Runset_status(), rand16Runset_status(), 0x4000|rand16Runset_status()&0x0fff, 0x8000|rand16Runset_status()&0x3fff, rand48Runset_status())
}
func rand16Runset_status() int {
	return int(time.Now().UnixNano()>>16) & 0xffff
}
func rand32Runset_status() uint32 {
	return uint32(time.Now().UnixNano()>>8) & 0xffffffff
}
func rand48Runset_status() int64 {
