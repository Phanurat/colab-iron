...
func generateUUIDpreC0mment6_for_reel() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment6_for_reel() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}

func generateHex32preC0mment6_for_reel() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment6_for_reel(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	fmt.Println("🌐 ใช้ Proxy:", proxyAddr) // ✅ เพิ่ม debug แสดง proxy ที่ใช้อยู่
	host := "graph.facebook.com"
	//	address := host + ":443"
	clientDocID := "245135309338835500871888555"
	friendlyName := "FamilyNonUserMemberTagQuery"
	connToken := generateHex32preC0mment6_for_reel()
	...
	traceID := generateUUIDpreC0mment6_for_reel()
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment6_for_reel())
