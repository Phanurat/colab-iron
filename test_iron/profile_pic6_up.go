...
func randomExcellentBandwidthRunprofile_pic6_up() string {
...
func findSmallestImagePathRunprofile_pic6_up(folder string) string {
	var smallest string
...
func Runprofile_pic6_up(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	profileFolder := filepath.Join(folder, "profile_photo")
	imagePath := findSmallestImagePathRunprofile_pic6_up(profileFolder)
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthRunprofile_pic6_up())
	...
	// dbApp := filepath.Join(folder, "photo_id.db")
	// fmt.Println("📂 เปิดไฟล์ฐานข้อมูลจาก path:", dbApp)

	// coverDB, err := sql.Open("sqlite3", dbApp)
	// if err != nil {
	// 	fmt.Println("❌ เปิดฐานข้อมูลไม่สำเร็จ: " + err.Error())
	// 	return
	// }
	// defer coverDB.Close()
	...
	// --- เตรียมพาธ SQLite ที่รันได้ทุกเครื่อง ---
	//	dbPath := filepath.Join(".", "photo_id.db")
	//	fmt.Println("📂 เปิดไฟล์ฐานข้อมูลจาก path:", dbPath)
	...
	_, err = db.Exec(`INSERT INTO profile_photo_id_table (pic_id) VALUES (?)`, photoID)
	if err != nil {
		fmt.Println("❌ INSERT pic_id ไม่สำเร็จ: " + err.Error())
		return
	}
	fmt.Println("💾 บันทึก photo_id ลง DB แล้ว:", photoID)
