...
func randomExcellentBandwidthcover_pic1up() string {
...
func findSmallestImagePathcover_pic1up(folder string) string {
	var smallest string
...
func Runcover_pic1up(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	imagePath := findSmallestImagePathcover_pic1up(profileFolder)
	...
	//	req.Header.Set("X-FB-Connection-Bandwidth", "30036802")
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthcover_pic1up())
	req.Header.Set("x-fb-connection-quality", "EXCELLENT")
	...
	// dbApp := filepath.Join(folder, "cover_photo_id.db")
	...
	// // _, err = coverDB.Exec(`DELETE FROM cover_photo_id_table`)
	// // if err != nil {
	// // 	fmt.Println("❌ ลบข้อมูลเก่าล้มเหลว: " + err.Error())
	// // 	return
	// // }
	...
	_, err = db.Exec(`INSERT INTO cover_photo_id_table (pic_id) VALUES (?)`, photoID)
	if err != nil {
