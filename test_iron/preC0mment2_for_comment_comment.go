...
func extractPostIDpreC0mment2_for_comment_comment(rawurl string) (string, error) {
...
func generateFeedbackIDpreC0mment2_for_comment_comment(postID string) string {
	return base64.StdEncoding.EncodeToString([]byte("feedback:" + postID))
}

func randomExcellentBandwidthpreC0mment2_for_comment_comment() string {
...
func generateUUIDpreC0mment2_for_comment_comment() string {
	return uuid.New().String()
}

func generateHex32preC0mment2_for_comment_comment() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment2_for_comment_comment(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	connToken := generateHex32preC0mment2_for_comment_comment()
	...
	postID, err := extractPostIDpreC0mment2_for_comment_comment(link)
	if err != nil {
		fmt.Println("❌ ขุด postID ล้มเหลว: " + err.Error())
	}
	feedbackID := generateFeedbackIDpreC0mment2_for_comment_comment(postID)
	traceID := generateUUIDpreC0mment2_for_comment_comment()
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment2_for_comment_comment())
