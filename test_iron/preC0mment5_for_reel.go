...
func extractPostIDpreC0mment5_for_reel(rawurl string) (string, error) {
	re := []*regexp.Regexp{
...
func generateFeedbackIDpreC0mment5_for_reel(postID string) string {
	return base64.StdEncoding.EncodeToString([]byte("feedback:" + postID))
}

func generateUUIDpreC0mment5_for_reel() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment5_for_reel() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}

func generateHex32preC0mment5_for_reel() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment5_for_reel(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	//textValue := "hkh"
	connToken := generateHex32preC0mment5_for_reel()
	...
	err = db.QueryRow("SELECT comment_text, link FROM like_reel_and_comment_reel_table LIMIT 1").Scan(&commentText, &link)
	if err != nil {
		fmt.Println("❌ ดึงลิงก์ไม่สำเร็จ: " + err.Error())
		return
	}

	postID, err := extractPostIDpreC0mment5_for_reel(link)
	if err != nil {
		fmt.Println("❌ ขุด postID ล้มเหลว: " + err.Error())
		return
	}
	feedbackID := generateFeedbackIDpreC0mment5_for_reel(postID)
	traceID := generateUUIDpreC0mment5_for_reel()
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment5_for_reel())
