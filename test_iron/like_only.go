...
var reactionIDslike_only = map[string]string{
...
func generateLikeMetalike_only(postID string) (string, string, string, string) {
...
func randomExcellentBandwidthlike_only() string {
...
func extractFacebookIDslike_only(rawurl string) (string, string, error) {
	...
	if ownerID == "" {
		if match := reUsername.FindStringSubmatch(rawurl); len(match) > 1 {
			username := match[1]
			if isNumericlike_only(username) {
				ownerID = username
			} else {
				fbid, err := getFBIDFromUsernamelike_only(username)
				if err != nil {
					return "", "", err
				}
				ownerID = fbid
...
func isNumericlike_only(s string) bool {
	for _, r := range s {
		if r < '0' || r > '9' {
			return false
		}
...
func getFBIDFromUsernamelike_only(username string) (string, error) {
	client := &http.Client{
...
func Runlike_only(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	err = db.QueryRow("SELECT reaction_type, link FROM like_only_table LIMIT 1").Scan(&reactionType, &link)
	if err != nil {
		panic("❌ ดึงลิงก์จาก comments ไม่สำเร็จ: " + err.Error())
	}

	ownerID, postID, err := extractFacebookIDslike_only(link)
	if err != nil {
		panic("❌ ขุด owner_id/post_id ไม่สำเร็จ: " + err.Error())
	}

	reactionID := reactionIDslike_only[reactionType]
	feedbackIDB64, clientMutationID, timestamp, qid := generateLikeMetalike_only(postID)
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthlike_only())
	...
	_, err = db.Exec("INSERT INTO respond_for_like_only_table (respond_txt) VALUES (?)", string(bodyResp))
	if err != nil {
		fmt.Println("❌ บันทึก response ลงตาราง respond_for_like_only_table ไม่สำเร็จ:", err)
	} else {
		fmt.Println("💾 บันทึก response แล้วลงตาราง respond_for_like_only_table")
	}

	_, err = db.Exec("DELETE FROM like_only_table WHERE link = ?", link) //db.Exec("DELETE FROM uid_table WHERE user_id = ?", uid)
	if err != nil {
