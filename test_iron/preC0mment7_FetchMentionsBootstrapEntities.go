...
func generateUUIDpreC0mment7_FetchMentionsBootstrapEntities() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment7_FetchMentionsBootstrapEntities() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}

func generateHex32preC0mment7_FetchMentionsBootstrapEntities() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment7_FetchMentionsBootstrapEntities(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	fmt.Println("🌐 ใช้ Proxy:", proxyAddr) // ✅ เพิ่ม debug แสดง proxy ที่ใช้อยู่

	host := "graph.facebook.com"
	//	address := host + ":443"
	clientDocID := "40654884719110375699959369431"
	friendlyName := "FetchMentionsBootstrapEntities"
	connToken := generateHex32preC0mment7_FetchMentionsBootstrapEntities()
	...
	traceID := generateUUIDpreC0mment7_FetchMentionsBootstrapEntities()

	variables := `{"enable_delta_update":true,"enable_update_bootstrap":true,"timestamp":0,"thumbnail_size":105,"alternate_name_enabled":true}`
	...
	//	req.Header.Set("x-fb-session-id", "nid=yTInr91goUUA;tid=4912;nc=0;fc=2;bc=2;cid=6cadeaa7d88b0a696bafd896c8c10410")
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment7_FetchMentionsBootstrapEntities())
