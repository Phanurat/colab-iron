...
func generateFeedbackIDpreC0mment1_for_comment_comment(postID string) string {
	feedbackID := "feedback:" + postID
	return base64.StdEncoding.EncodeToString([]byte(feedbackID))
}

func extractPostIDpreC0mment1_for_comment_comment(rawurl string) (string, error) {
	var postID string
...
func isNumericpreC0mment1_for_comment_comment(s string) bool {
	for _, r := range s {
		if r < '0' || r > '9' {
			return false
		}
...
func randomExcellentBandwidthpreC0mment1_for_comment_comment() string {
...
func RunpreC0mment1_for_comment_comment(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	postID, err := extractPostIDpreC0mment1_for_comment_comment(link)
	if err != nil || postID == "" {
		fmt.Println("❌ ดึง postID ไม่สำเร็จจากลิงก์: " + link)
		return
	}

	feedbackID := generateFeedbackIDpreC0mment1_for_comment_comment(postID)
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment1_for_comment_comment()) //เพิ่มเข้าไป
