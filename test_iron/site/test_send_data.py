...
# def save_promp_id(index):
#     try:
#         # บันทึกค่า index ลงในไฟล์
#         with open("promp_id.txt", "w") as f:
#             f.write(str(index))
#         print(f"✅ Saved prompt id {index} to file.")
#     except Exception as e:
#         print(f"❌ Error saving prompt id to file: {e}")

def get_charactor_by_fixed_index(index):
    try:
        conn = sqlite3.connect("./promt.db")
        cur = conn.cursor()
        cur.execute("SELECT * FROM charactor LIMIT 1")
        row = cur.fetchone()
        columns = [desc[0] for desc in cur.description]
        conn.close()
        
        # ตรวจสอบว่าค่า index ไม่เกินจำนวนคอลัมน์
        if index >= len(columns):
            print(f"❌ Index {index} is out of range. Total columns: {len(columns)}")
            return None
        
        print(f"🧠 Using prompt index {index}: {columns[index]}")
        
        # ส่ง index ไปให้ฟังก์ชัน save_promp_id
        # save_promp_id(index)
