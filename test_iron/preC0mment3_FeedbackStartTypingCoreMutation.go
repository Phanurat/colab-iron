...
func extractPostIDpreC0mment3_FeedbackStartTypingCoreMutation(rawurl string) (string, error) {
	re := []*regexp.Regexp{
...
func generateFeedbackIDpreC0mment3_FeedbackStartTypingCoreMutation(postID string) string {
	return base64.StdEncoding.EncodeToString([]byte("feedback:" + postID))
}

func generateUUIDpreC0mment3_FeedbackStartTypingCoreMutation() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment3_FeedbackStartTypingCoreMutation() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}
func generateHex32preC0mment3_FeedbackStartTypingCoreMutation() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment3_FeedbackStartTypingCoreMutation(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	connToken := generateHex32preC0mment3_FeedbackStartTypingCoreMutation()
	...
	postID, err := extractPostIDpreC0mment3_FeedbackStartTypingCoreMutation(link)
	if err != nil {
		fmt.Println("❌ ขุด postID ล้มเหลว: " + err.Error())
		return
	}
	feedbackID := generateFeedbackIDpreC0mment3_FeedbackStartTypingCoreMutation(postID)
	traceID := generateUUIDpreC0mment3_FeedbackStartTypingCoreMutation()
	clientMutationID := generateUUIDpreC0mment3_FeedbackStartTypingCoreMutation()
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment3_FeedbackStartTypingCoreMutation())
