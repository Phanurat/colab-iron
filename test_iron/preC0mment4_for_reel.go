...
func extractPostIDpreC0mment4_for_reel(rawurl string) (string, error) {
	re := []*regexp.Regexp{
...
func generateFeedbackTargetIDpreC0mment4_for_reel(postID string) string {
	id := "feedback:" + postID
	return base64.StdEncoding.EncodeToString([]byte(id))
}

func generateUUIDpreC0mment4_for_reel() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment4_for_reel() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}

func generateHex32preC0mment4_for_reel() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment4_for_reel(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	fmt.Println("🌐 ใช้ Proxy:", proxyAddr) // ✅ เพิ่ม debug แสดง proxy ที่ใช้อยู่
	host := "graph.facebook.com"
	//	address := host + ":443"
	clientDocID := "40414289251135926563767751059"
	friendlyName := "MentionsSuggestionQuery"
	connToken := generateHex32preC0mment4_for_reel()
	...
	postID, err := extractPostIDpreC0mment4_for_reel(link)
	if err != nil {
		fmt.Println("❌ ขุด postID ไม่สำเร็จ: " + err.Error())
		return
	}
	feedbackTargetID := generateFeedbackTargetIDpreC0mment4_for_reel(postID)
	traceID := generateUUIDpreC0mment4_for_reel()
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment4_for_reel())
