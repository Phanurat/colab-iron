...
func extractPostIDpreC0mment8_for_comment_comment(rawurl string) (string, error) {
	re := []*regexp.Regexp{
...
func generateFeedbackIDpreC0mment8_for_comment_comment(postID string) string {
	return base64.StdEncoding.EncodeToString([]byte("feedback:" + postID))
}

func generateUUIDpreC0mment8_for_comment_comment() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment8_for_comment_comment() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}

func generateHex32preC0mment8_for_comment_comment() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment8_for_comment_comment(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	...
	connToken := generateHex32preC0mment8_for_comment_comment()
	...
	postID, err := extractPostIDpreC0mment8_for_comment_comment(link)
	if err != nil {
		fmt.Println("❌ ขุด postID ไม่สำเร็จ: " + err.Error())
		return
	}
	feedbackID := generateFeedbackIDpreC0mment8_for_comment_comment(postID)
	traceID := generateUUIDpreC0mment8_for_comment_comment()
	mutationID := generateUUIDpreC0mment8_for_comment_comment()
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment8_for_comment_comment())
