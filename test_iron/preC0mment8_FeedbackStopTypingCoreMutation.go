...
func extractPostIDpreC0mment8_FeedbackStopTypingCoreMutation(rawurl string) (string, error) {
	re := []*regexp.Regexp{
...
func generateFeedbackIDpreC0mment8_FeedbackStopTypingCoreMutation(postID string) string {
	return base64.StdEncoding.EncodeToString([]byte("feedback:" + postID))
}

func generateUUIDpreC0mment8_FeedbackStopTypingCoreMutation() string {
	return uuid.New().String()
}

func randomExcellentBandwidthpreC0mment8_FeedbackStopTypingCoreMutation() string {
	rand.Seed(time.Now().UnixNano())
	return strconv.Itoa(rand.Intn(15000000) + 20000000)
}

func generateHex32preC0mment8_FeedbackStopTypingCoreMutation() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// connToken := generateHex32()

func RunpreC0mment8_FeedbackStopTypingCoreMutation(tlsConns *TLSConnections, proxyAddr, proxyAuth string) {
	fmt.Println("🌐 ใช้ Proxy:", proxyAddr) // ✅ เพิ่ม debug แสดง proxy ที่ใช้อยู่

	host := "graph.facebook.com"
	//	address := host + ":443"
	clientDocID := "5811160523257420980347697991"
	friendlyName := "FeedbackStopTypingCoreMutation"
	connToken := generateHex32preC0mment8_FeedbackStopTypingCoreMutation()
	...
	postID, err := extractPostIDpreC0mment8_FeedbackStopTypingCoreMutation(link)
	if err != nil {
		fmt.Println("❌ ขุด postID ไม่สำเร็จ: " + err.Error())
		return
	}
	feedbackID := generateFeedbackIDpreC0mment8_FeedbackStopTypingCoreMutation(postID)
	traceID := generateUUIDpreC0mment8_FeedbackStopTypingCoreMutation()
	mutationID := generateUUIDpreC0mment8_FeedbackStopTypingCoreMutation()

	variables := fmt.Sprintf(`{"input":{"client_mutation_id":"%s","feedback_id":"%s","actor_id":"%s"}}`, mutationID, feedbackID, actorID)
	...
	//	req.Header.Set("x-fb-session-id", "nid=yTInr91goUUA;tid=4930;nc=0;fc=2;bc=2;cid=919b58f177dc55125912687a8020fac1")
	...
	req.Header.Set("x-fb-connection-bandwidth", randomExcellentBandwidthpreC0mment8_FeedbackStopTypingCoreMutation())
